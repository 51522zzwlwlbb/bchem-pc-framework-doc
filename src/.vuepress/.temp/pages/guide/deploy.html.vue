<template><div><h3 id="构建" tabindex="-1"><a class="header-anchor" href="#构建" aria-hidden="true">#</a> 构建</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>构建打包成功之后，会在根目录生成 dist 文件夹，里面就是构建打包好的文件</p>
<h3 id="旧版浏览器兼容" tabindex="-1"><a class="header-anchor" href="#旧版浏览器兼容" aria-hidden="true">#</a> 旧版浏览器兼容</h3>
<p>在 <strong>.env.production</strong> 内</p>
<p>设置 <code v-pre>VITE_LEGACY=true</code> 即可打包出兼容旧版浏览器的代码</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>VITE_LEGACY <span class="token operator">=</span> <span class="token boolean">true</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="预览" tabindex="-1"><a class="header-anchor" href="#预览" aria-hidden="true">#</a> 预览</h3>
<p>发布之前可以在本地进行预览，有多种方式，这里介绍两种</p>
<p><strong>不能直接打开构建后的 html 文件</strong></p>
<ul>
<li>使用项目自定的命令进行预览(推荐)</li>
</ul>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 先打包在进行预览</span>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 1.全局安装live-server</span>
...
<p>如果你的构建文件很大，可以通过项目内置 <a href="https://github.com/doesdev/rollup-plugin-analyzer" target="_blank" rel="noopener noreferrer">rollup-plugin-analyzer<ExternalLinkIcon/></a> 插件进行代码体积分析，从而优化你的代码。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>运行之后，在自动打开的页面可以看到具体的体积分布，以分析哪些依赖有问题。</p>
<div class="hint-container tip">
<p class="hint-container-title">提示</p>
<p>左上角可以切换 显示 gzip 或者 brotli</p>
</div>
<h3 id="压缩" tabindex="-1"><a class="header-anchor" href="#压缩" aria-hidden="true">#</a> 压缩</h3>
<h4 id="开启-gzip-压缩" tabindex="-1"><a class="header-anchor" href="#开启-gzip-压缩" aria-hidden="true">#</a> 开启 gzip 压缩</h4>
<p>开启 gzip，并配合 nginx 的 <code v-pre>gzip_static</code> 功能可以大大加快页面访问速度</p>
<div class="hint-container tip">
<p class="hint-container-title">提示</p>
<p>只需开启 VITE_BUILD_COMPRESS='gzip' 即可在打包的同时生成 .gz 文件</p>
</div>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 根据自己路径来配置更改</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="同时开启-gzip-与-brotli" tabindex="-1"><a class="header-anchor" href="#同时开启-gzip-与-brotli" aria-hidden="true">#</a> 同时开启 gzip 与 brotli</h4>
<p>只需开启 <code v-pre>VITE_BUILD_COMPRESS='brotli,gzip'</code> 即可在打包的同时生成 <code v-pre>.gz</code> 和 <code v-pre>.br</code> 文件。</p>
<h4 id="gzip-与-brotli-在-nginx-内的配置" tabindex="-1"><a class="header-anchor" href="#gzip-与-brotli-在-nginx-内的配置" aria-hidden="true">#</a> gzip 与 brotli 在 nginx 内的配置</h4>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>http <span class="token punctuation">{</span>
