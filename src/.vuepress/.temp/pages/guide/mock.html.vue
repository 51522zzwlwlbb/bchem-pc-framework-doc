...
<p>如果前端应用和后端接口服务器没有运行在同一个主机上，你需要在开发环境下将接口请求代理到接口服务器。</p>
<p>如果是同一个主机，可以直接请求具体的接口地址。</p>
<h3 id="配置" tabindex="-1"><a class="header-anchor" href="#配置" aria-hidden="true">#</a> 配置</h3>
<p>开发环境时候，接口地址在项目根目录下</p>
<p><code v-pre>.env.development</code> 文件配置</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># vite 本地跨域代理</span>
...
<ul>
<li><code v-pre>.env</code> 文件中的字段如果是字符串，则无需加引号，默认全部为字符串</li>
<li><code v-pre>VITE_PROXY</code> 不能换行</li>
</ul>
</div>
<h3 id="跨域处理" tabindex="-1"><a class="header-anchor" href="#跨域处理" aria-hidden="true">#</a> 跨域处理</h3>
<p>如果你在 <code v-pre>src/api/</code> 下面的接口为下方代码，且 <code v-pre>.env.development</code> 文件配置如下注释，则在控制台看到的地址为 <code v-pre>http://localhost:8001/api/login</code>。</p>
<p>由于 <code v-pre>/api</code> 匹配到了设置的 <code v-pre>VITE_PROXY</code>，所以上方实际是请求 <code v-pre>http://localhost:8000/login</code>，这样同时也解决了跨域问题。（8001 为项目端口号，<code v-pre>http://localhost:8000</code> 为 <code v-pre>PROXY</code> 代理的目标地址）</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// .env.development</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="没有跨域时的配置" tabindex="-1"><a class="header-anchor" href="#没有跨域时的配置" aria-hidden="true">#</a> 没有跨域时的配置</h3>
<p>如果没有跨域问题，可以直接忽略 <code v-pre>VITE_PROXY</code> 配置，直接将接口地址设置在 <code v-pre>VITE_GLOB_API_URL</code></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 例如接口地址为 http://localhost:8000 则</span>
<span class="token assign-left variable">VITE_GLOB_API_URL</span><span class="token operator">=</span>http://localhost:8000
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>如果有跨域问题，将 <code v-pre>VITE_GLOB_API_URL</code> 设置为跟 <code v-pre>VITE_PROXY</code> 内其中一个数组的第一个项一致的值即可。</p>
<p>下方的接口地址设置为 <code v-pre>/api</code>，当请求发出的时候会经过 <code v-pre>Vite</code> 的 <code v-pre>proxy</code> 代理，匹配到了我们设置的 <code v-pre>VITE_PROXY</code> 规则，将 <code v-pre>/api</code> 转化为 <code v-pre>http://localhost:8000</code> 进行请求</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 例如接口地址为 http://localhost:8000 则</span>
...
<span class="token comment"># 接口地址</span>
<span class="token assign-left variable">VITE_GLOB_API_URL</span><span class="token operator">=</span>/api
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="跨域原理解析" tabindex="-1"><a class="header-anchor" href="#跨域原理解析" aria-hidden="true">#</a> 跨域原理解析</h3>
<p>在 <code v-pre>vite.config.ts</code> 配置文件中，提供了 <code v-pre>server</code> 的 <code v-pre>proxy</code> 功能，用于代理 API 请求。</p>
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code>server<span class="token operator">:</span> <span class="token punctuation">{</span>
...
<p>从浏览器控制台的 Network 看，请求是 <code v-pre>http://localhost:8000/api/xxx</code>，这是因为 <code v-pre>proxy</code> 配置不会改变本地请求的 <code v-pre>url</code>。</p>
</div>
<h2 id="生产环境" tabindex="-1"><a class="header-anchor" href="#生产环境" aria-hidden="true">#</a> 生产环境</h2>
<p>生产环境接口地址在项目根目录下 <code v-pre>.env.production</code> 文件配置。</p>
<p>生产环境接口地址值需要修改 <code v-pre>VITE_GLOB_API_URL</code>，如果出现跨域问题，可以使用 <code v-pre>nginx</code> 或者后台开启 <code v-pre>cors</code> 进行处理</p>
<div class="hint-container tip">
<p class="hint-container-title">打包后如何进行地址修改?</p>
<p><code v-pre>VITE_GLOB_*</code> 开头的变量会在打包的时候注入 <code v-pre>_app.config.js</code> 文件内。</p>
<p>在 <code v-pre>dist/_app.config.js</code> 修改相应的接口地址后刷新页面即可，不需要在根据不同环境打包多次，一次打包可以用于多个不同接口环境的部署。</p>
</div>
<h2 id="接口请求" tabindex="-1"><a class="header-anchor" href="#接口请求" aria-hidden="true">#</a> 接口请求</h2>
<p>在 hgb-admin 中:</p>
<ol>
<li>页面交互操作；</li>
<li>调用统一管理的 api 请求函数；</li>
<li>使用封装的 axios.ts 发送请求；</li>
<li>获取服务端返回数据</li>
<li>更新 data；</li>
</ol>
<p>接口统一存放于 <code v-pre>src/api/</code> 下面管理</p>
<p>以登陆接口为例:</p>
<p>在 <code v-pre>src/api/</code> 内新建模块文件，其中参数与返回值最好定义一下类型，方便校验。虽然麻烦，但是后续维护字段很方便。</p>
<div class="hint-container tip">
<p class="hint-container-title">提示</p>
<p>类型定义文件可以抽取出去统一管理，具体参考项目，至于参数类型校验，可以单独抽离 model 引入</p>
</div>
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token keyword">import</span> http <span class="token keyword">from</span> <span class="token string">'@/utils/http/axios'</span><span class="token punctuation">;</span>
...
<span class="token keyword">export</span> <span class="token keyword">function</span> <span class="token function">getTableList</span><span class="token punctuation">(</span>params<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="axios-配置" tabindex="-1"><a class="header-anchor" href="#axios-配置" aria-hidden="true">#</a> axios 配置</h2>
<p>axios 请求封装存放于 <code v-pre>src/utils/http/axios</code> 文件夹内部</p>
<p>除 <code v-pre>index.ts</code> 文件内容需要根据项目自行修改外，其余文件无需修改</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code>├── Axios<span class="token punctuation">.</span>ts <span class="token comment">// axios实例</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="index-ts-配置说明" tabindex="-1"><a class="header-anchor" href="#index-ts-配置说明" aria-hidden="true">#</a> index.ts 配置说明</h3>
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token keyword">const</span> axios <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">VAxios</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="transform-数据处理说明" tabindex="-1"><a class="header-anchor" href="#transform-数据处理说明" aria-hidden="true">#</a> transform 数据处理说明</h4>
<p>类型定义，见 <code v-pre>axiosTransform.ts</code> 文件</p>
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token keyword">export</span> <span class="token keyword">abstract</span> <span class="token keyword">class</span> <span class="token class-name">AxiosTransform</span> <span class="token punctuation">{</span>
  <span class="token doc-comment comment">/**
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>项目默认 <code v-pre>transform</code> 处理逻辑，可以根据各自项目进行处理。一般需要更改的部分为下方代码，见代码注释说明</p>
...
<p>项目接口默认为 <code v-pre>Json</code> 参数格式，即 <code v-pre>headers: { 'Content-Type': ContentTypeEnum.JSON }</code>,</p>
<p>如果需要更改为 <code v-pre>form-data</code> 格式，更改 <code v-pre>headers</code> 的 <code v-pre>'Content-Type 为 ContentTypeEnum.FORM_URLENCODED</code> 即可</p>
<h3 id="多个接口地址" tabindex="-1"><a class="header-anchor" href="#多个接口地址" aria-hidden="true">#</a> 多个接口地址</h3>
<p>当项目中需要用到多个接口地址时, 可以在 <code v-pre>src/utils/http/axios/index.ts</code> 导出多个 <code v-pre>axios</code> 实例</p>
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token comment">// 目前只导出一个默认实例，接口地址对应的是环境变量中的 VITE_GLOB_API_URL 接口地址</span>
...
<p>如果不需要 url 上面默认携带的时间戳参数 ?_t=xxx</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> axios <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">VAxios</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span><span class="token punctuation">,</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="mock-服务" tabindex="-1"><a class="header-anchor" href="#mock-服务" aria-hidden="true">#</a> Mock 服务</h2>
<p><code v-pre>Mock</code> 数据是前端开发过程中必不可少的一环，是分离前后端开发的关键链路。通过预先跟服务器端约定好的接口，模拟请求数据甚至逻辑，能够让前端开发独立自主，不会被服务端的开发进程所阻塞。</p>
<p>本项目使用 <a href="https://github.com/vbenjs/vite-plugin-mock" target="_blank" rel="noopener noreferrer">vite-plugin-mock<ExternalLinkIcon/></a> 来进行 <code v-pre>mock</code> 数据处理。项目内 <code v-pre>mock</code> 服务分本地和线上。</p>
<h3 id="本地-mock" tabindex="-1"><a class="header-anchor" href="#本地-mock" aria-hidden="true">#</a> 本地 Mock</h3>
<p>本地 mock 采用 Node.js 中间件进行参数拦截（不采用 mock.js 的原因是本地开发看不到请求参数和响应结果）。</p>
<h3 id="如何新增-mock-接口" tabindex="-1"><a class="header-anchor" href="#如何新增-mock-接口" aria-hidden="true">#</a> 如何新增 mock 接口</h3>
<p>如果你想添加 mock 数据，只要在根目录下找到 mock 文件，添加对应的接口，对其进行拦截和模拟数据。</p>
<p>在 mock 文件夹内新建文件</p>
<div class="hint-container tip">
<p class="hint-container-title">提示</p>
<p>文件新增后会自动更新，不需要手动重启，可以在代码控制台查看日志信息 mock 文件夹内会自动注册，排除以_开头的文件夹及文件</p>
</div>
<p><strong>例：</strong></p>
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> Random <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'mockjs'</span><span class="token punctuation">;</span>
...
<p class="hint-container-title">提示</p>
<p>mock 的值可以直接使用 <a href="https://github.com/nuysoft/Mock/wiki" target="_blank" rel="noopener noreferrer">mockjs<ExternalLinkIcon/></a> 的语法。</p>
</div>
<h3 id="接口格式" tabindex="-1"><a class="header-anchor" href="#接口格式" aria-hidden="true">#</a> 接口格式</h3>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="参数获取" tabindex="-1"><a class="header-anchor" href="#参数获取" aria-hidden="true">#</a> 参数获取</h3>
<p><strong>GET</strong> 接口： <code v-pre>({ query }) =&gt; { }</code></p>
<p><strong>POST</strong> 接口： <code v-pre>({ body }) =&gt; { }</code></p>
<h3 id="匹配" tabindex="-1"><a class="header-anchor" href="#匹配" aria-hidden="true">#</a> 匹配</h3>
<p>在 <code v-pre>src/api</code> 下面，如果接口匹配到 mock，则会优先使用 mock 进行响应</p>
<div class="language-typescript line-numbers-mode" data-ext="ts"><pre v-pre class="language-typescript"><code><span class="token keyword">import</span> http <span class="token keyword">from</span> <span class="token string">'@/utils/http/axios'</span><span class="token punctuation">;</span>
...
<p>当后台接口已经开发完成，只需要将相应的 mock 函数去掉即可。</p>
<p>以上方接口为例，假如后台接口 login 已经开发完成，则只需要删除/注释掉下方代码即可</p>
...
<p class="hint-container-title">为什么通过插件注入代码而不是直接在 main.ts 内插入</p>
<p>在插件内通过 injectCode 插入代码，方便控制 mockjs 是否被打包到最终代码内。如果在 main.ts 内判断，如果关闭了 mock 功能，mockjs 也会打包到构建文件内，这样会增加打包体积。</p>
</div>
</div></template>
